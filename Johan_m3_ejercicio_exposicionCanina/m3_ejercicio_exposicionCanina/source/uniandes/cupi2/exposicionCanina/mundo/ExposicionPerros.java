...
public class ExposicionPerros
{
    ...
    public void ordenarPorRaza( )
    {
    	...
    	for( int i = 0; i < perros.size()-1; i++ )
        {
            ...
            {
            	Perro posJ= (Perro)perros.get(j);
            	Perro posJMas1= (Perro)perros.get(j+1);
                // Intercambia si están en el orden equivocado
                if( posJ.compararPorRaza(posJMas1)>0 )
                	
                {
                    Perro temp = posJ;
                    perros.set(j, posJMas1) ;
                    perros.set(j+1, temp);
                }
            }
        }
    ...
    public void ordenarPorNombre( )
    {
    	...
    	for( int i = 1; i < perros.size(); i++ )
        {
            ...
            for( int j = i; j > 0 ; j-- )
            {
            	Perro pos1= (Perro)perros.get(j-1);
            	Perro pos2= (Perro)perros.get(j);
            	if(pos1.compararPorNombre(pos2)>0) {
            		Perro temp = pos2;
            		perros.set(j,pos1);
            		perros.set(j-1,temp);
    ...
    public void ordenarPorPuntos( )
    {
    	...
    	for( int i = 0; i < perros.size()-1; i++ )
        {
            for( int j = 0; j < perros.size()-i - 1; j++ )
            {
            	...
            	Perro posJMas1= (Perro)perros.get(j+1);
                // Intercambia si están en el orden equivocado
                if( posJ.compararPorPuntos(posJMas1)>0 )
    ...
    public void ordenarPorEdad( )
    {
    	...
    	for( int i = 0; i < perros.size()-1; i++ )
        {
            ...
            for( int j = i+1; j < perros.size(); j++ )
            {
            	...
            	Perro posJMas1= (Perro)perros.get(j);
                // Intercambia si están en el orden equivocado
                if( menor.compararPorEdad(posJMas1)>0 ){
                	menor=posJMas1;
                	pos=j;
                }
